...
class VotingMethodEvaluator:
    ...
    def kendall_tau_b(self, actual: List[int], simulated: List[int]) -> float:
        ...
        # 转换为排名
        actual_ranks = self._convert_to_ranks(actual)
        simulated_ranks = self._convert_to_ranks(simulated)
        ...
        for i in range(n):
            for j in range(i+1, n):
                sign_actual = np.sign(actual_ranks[i] - actual_ranks[j])
                sign_sim = np.sign(simulated_ranks[i] - simulated_ranks[j])
                
                if sign_actual * sign_sim > 0:
                    n_concordant += 1
                elif sign_actual * sign_sim < 0:
                    n_discordant += 1
        
        # 调整相同排名的项
        n0 = n * (n - 1) / 2
        n1 = self._count_ties(actual_ranks)
        n2 = self._count_ties(simulated_ranks)
        
        if n0 == n1 or n0 == n2:
            return 0.0
        
        tau_b = (n_concordant - n_discordant) / np.sqrt((n0 - n1) * (n0 - n2))
        return tau_b
    ...
    def grey_relational_analysis(self, sequences: List[np.ndarray]) -> np.ndarray:
        ...
        reference_norm = (reference - np.min(reference)) / (np.max(reference) - np.min(reference) + 1e-10)
    ...
    def calculate_criteria(self, method_results: Dict) -> Dict[str, float]:
        ...
        criteria_values['历史一致性'] = self.kendall_tau_b(
            method_results['actual_elimination'],
            method_results['simulated_order']
        )
        ...
        for week_data in method_results.get('weekly_details', []):
            if 'fan_ranks' in week_data and 'combined_ranks' in week_data:
                fan_inf = abs(self.spearman_correlation(
                    week_data['fan_ranks'],
                    week_data['combined_ranks']
                ))
                fan_influences.append(fan_inf)
        
        criteria_values['粉丝影响力'] = np.mean(fan_influences) if fan_influences else 0.5
        ...
        for week_data in method_results.get('weekly_details', []):
            if 'judge_ranks' in week_data and 'fan_ranks' in week_data:
                gap = np.abs(np.array(week_data['judge_ranks']) - np.array(week_data['fan_ranks']))
...
class TOPSISDecisionModel:
    ...
    def determine_ideal_solutions(self, matrix: np.ndarray, criteria_types: List[str]) -> Tuple[np.ndarray, np.ndarray]:
        """确定理想解和负理想解"""
    ...
    def calculate_distances(self, matrix: np.ndarray, ideal: np.ndarray, negative: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        """计算到理想解和负理想解的距离"""
    ...
    def calculate_closeness(self, d_plus: np.ndarray, d_minus: np.ndarray) -> np.ndarray:
        """计算相对贴近度"""
        return d_minus / (d_plus + d_minus)
    
    def topsis_evaluate(self, decision_matrix: np.ndarray, criteria_types: List[str], 
...
class SensitivityAnalyzer:
    ...
    def monte_carlo_sensitivity(self, decision_matrix: np.ndarray, criteria_types: List[str],
                               n_simulations: int = 1000, noise_level: float = 0.2) -> Dict:
        """蒙特卡洛敏感性分析"""
        ...
        for _ in range(n_simulations):
            ...
            result = topsis.topsis_evaluate(decision_matrix, criteria_types, perturbed_weights)
    ...
    def criterion_sensitivity(self, decision_matrix: np.ndarray, criteria_types: List[str],
                            base_weights: np.ndarray, variation_range: float = 0.3) -> Dict:
        """准则敏感性分析"""
        n_criteria = len(criteria_types)
        ...
        for j in range(n_criteria):
            ...
            for delta in np.linspace(-variation_range, variation_range, 11):
                ...
                # 执行TOPSIS
                topsis = TOPSISDecisionModel()
                result = topsis.topsis_evaluate(decision_matrix, criteria_types, weights)
...
class FuzzyTOPSIS:
    ...
    def triangular_fuzzy_number(self, lower: float, middle: float, upper: float) -> Tuple[float, float, float]:
        """创建三角模糊数"""
    ...
    def fuzzy_normalize(self, fuzzy_matrix: np.ndarray) -> np.ndarray:
        ...
        for i in range(n_alternatives):
            for j in range(n_criteria):
                ...
                normalized[i, j, :] = (l/max_upper[j], m/max_upper[j], u/max_upper[j])
        
        return normalized
    
    def fuzzy_distance(self, a: Tuple[float, float, float], b: Tuple[float, float, float]) -> float:
        """计算两个模糊数之间的距离"""
        l1, m1, u1 = a
        l2, m2, u2 = b
...
class ComprehensiveAnalysisFramework:
    ...
    def analyze_season(self, season_num: int) -> Dict:
        """分析单个赛季"""
        # 获取赛季数据
        from data_preparation import enhanced_prepare_season_data
    ...
    def generate_recommendation_report(self) -> Dict:
        ...
        for i, criterion in enumerate(self.evaluator.criteria_names):
            rank_score = criteria_df.iloc[0, i] if best_idx == 0 else criteria_df.iloc[1, i]
            percent_score = criteria_df.iloc[1, i] if best_idx == 1 else criteria_df.iloc[0, i]
            diff = rank_score - percent_score if best_idx == 0 else percent_score - rank_score
            
            if abs(diff) > 0.1:  # 显著差异
                reasons.append(f"{criterion}: {recommended_method}得分高{diff:.2f} (权重:{weights[i]:.2f})")
        ...
        report = {
            ...
            '权重分布': dict(zip(self.evaluator.criteria_names, 
            ...
            '敏感性分析': {
                ...
                '最敏感准则': self.evaluator.criteria_names[
                    analysis_results['criterion_sensitivity']['most_sensitive_criterion']
                ]
            }
        }
